...
test("KVLedger: unlock releases lock", async () => {
  ...
  // Assert: Verify lock is released (verifyLock with the old lockId should return false)
  const isLocked = await ledger.verifyLock(lockId);
...
test("KVLedger: verifyLock fails when not locked", async () => {
  ...
  // Act & Assert: Verify should fail with a non-matching lockId (since file is not locked, lock bytes are 0)
...
test("KVLedger: multiple lock attempts", async () => {
  ...
  // Assert: Second ledger should see it's locked (using wrong lockId)
  const canLock = await ledger2.verifyLock(BigInt(0));
